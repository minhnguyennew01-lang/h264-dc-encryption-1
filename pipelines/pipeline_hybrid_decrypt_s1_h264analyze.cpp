...
// Strip known algo suffixes to recover original video basename
static string strip_algo_suffix(const string& encrypted_path) {
    string name = get_basename(encrypted_path);
    static const char* suffixes[] = {
        ".s1_hybrid_fixed", ".s1_aes", ".s1_des", ".s1_rc4", nullptr
    };
...
int main(int argc, char* argv[]) {
    ...
    for (int a = 3; a < argc; a++) {
        if (string(argv[a]) == "--algo" && a + 1 < argc) {
            ...
            nalu_info_file = argv[a];  // legacy positional: nalu_info path
        }
    }
    ...
    cout << "📊 Strategy: S1 (Type 1 + Type 5)\n";
    ...
    string failed_path = encrypted_file + ".failed_s1.txt";
    ...
    cout << "\n✅ Decryption complete! (" << algo_name << " S1)\n";
