...
int main(int argc, char* argv[]) {
    ...
    std::cout << "[Strategy 2] Reading file...\n";
    ...
    std::cout << "[Strategy 2] File size: " << file_size << " bytes\n";

    // Extract DC BEFORE encryption
    std::cout << "[Strategy 2] Extracting DC coefficients BEFORE encryption...\n";
    int dc_count_before = extract_and_save_dc(h264_data, output_file + ".dc_before.txt", "BEFORE");

    // Parse and encrypt NALUs
    std::cout << "[Strategy 2] Extracting NALUs...\n";
    ...
    std::cout << "[Strategy 2] Found " << nalu_count << " NALUs\n";

    // Strategy 2: Encrypt ONLY I-frames (NALU type 5)
    std::cout << "[Strategy 2] Encrypting I-frames only...\n";
    std::vector<uint8_t> key_bytes(key.begin(), key.end());
    int encrypted_count = 0;
    for (size_t j = 0; j < nalus.size(); j++) {
        if (nalu_types[j] == 5) { // I-frame only
    ...
    std::cout << "[Strategy 2] Writing encrypted output...\n";
    ...
    std::cout << "[Strategy 2] Extracting DC coefficients AFTER encryption...\n";
    ...
    std::cout << "Encryption complete:\n"
              ...
              << "  Encrypted (I-frames): " << encrypted_count 
