...
- **Được sử dụng bởi**: Tất cả 3 strategies (S1, S2, S3)
- **Các hàm chính**:
...
#### 5. **pipeline_hybrid_s3_fixed.cpp** → Binary: `pipeline_hybrid_s3_fixed`
- **Chức năng**: Mã hóa mọi frame thứ 3 (Type 1 + Type 5 alternating)
- **Input**: `<video.h264>` + `<password>`
- **Output**: `<video.h264>.s3_hybrid_fixed` + `<video.h264>.s3_hybrid_fixed.meta`
- **Frames mã hóa**: ~6,055 frames (mỗi frame thứ 3)
- **Thời gian**: ~1.18 giây
- **Bảo mật**: ⭐⭐⭐⭐ (Cao)

**Trình tự xử lý trong S3**:
```
1. Read input video file
   ↓
2. Iterate through each NALU (with counter)
   ↓
3. For NALU count % 3 == 0 (every 3rd) with size ≥ 25 bytes:
   a. Encrypt DC coefficients (same as S1)
   ↓
4. Other frames: copy as-is
   ↓
5. Write all NALUs
   ↓
6. Write metadata
...
### 🔓 Pipeline Decryption (3 Strategies)

#### 6. **pipeline_hybrid_decrypt_s1_fixed.cpp** → Binary: `pipeline_hybrid_decrypt_s1_fixed`
- **Chức năng**: Giải mã S1 encrypted file
- **Input**: `<encrypted.s1_hybrid_fixed>` + `<password>`
- **Output**: `<encrypted>.s1_hybrid_fixed.decrypted`
- **Thời gian**: ~1.43 giây

#### 7. **pipeline_hybrid_decrypt_s2_fixed.cpp** → Binary: `pipeline_hybrid_decrypt_s2_fixed`
- **Chức năng**: Giải mã S2 encrypted file
- **Input**: `<encrypted.s2_hybrid_fixed>` + `<password>`
- **Output**: `<encrypted>.s2_hybrid_fixed.decrypted`
- **Thời gian**: ~1.16 giây

#### 8. **pipeline_hybrid_decrypt_s3_fixed.cpp** → Binary: `pipeline_hybrid_decrypt_s3_fixed`
- **Chức năng**: Giải mã S3 encrypted file
- **Input**: `<encrypted.s3_hybrid_fixed>` + `<password>`
- **Output**: `<encrypted>.s3_hybrid_fixed.decrypted`
- **Thời gian**: ~1.31 giây
...
**Output**: 6 executables (3 encrypt + 3 decrypt)
...
#### Option C: Strategy 3 (Mã hóa mọi frame thứ 3)
```bash
./pipeline_hybrid_s3_fixed output.h264 testkey

# Generates:
# - output.h264.s3_hybrid_fixed (encrypted video)
...
S3 Encryption Process (output):
├── Reading NALUs...
├── Frame [0]: NALU_COUNT % 3 == 0? → encrypted
├── Frame [1]: NALU_COUNT % 3 != 0 → NOT encrypted
├── Frame [2]: NALU_COUNT % 3 != 0 → NOT encrypted
├── Frame [3]: NALU_COUNT % 3 == 0? → encrypted
├── ...
├── Frames encrypted: 6,055 (every 3rd)
├── Frames passed through: 13,294
...
- ✅ Tất cả 3 strategies đều **PRODUCTION READY**
