...
%% Row 1: byte layout
\node[startcode] (sc) at (0,0) {\shortstack{Start code\\$4\,$B}};
\node[header, right=0pt of sc] (nh) {NAL header\\\(1\,\mathrm{B}\)};
\node[header, right=0pt of nh] (hdr) {Header)};
\node[dc, right=0pt of hdr] (dc) {\textbf{DC bytes}\\(25 B)};
...
\draw[<->, thick, orange!70!black]
  ...
  node[midway, below, font=\footnotesize] {\texttt{DC\_OFFSET, DC\_SIZE}};
