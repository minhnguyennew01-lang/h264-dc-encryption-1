...
3. **Updated** S1 and S2 pipelines to use pre-extracted NALU info (S3 removed from project)
...
The S3 strategy (every 3rd frame) has been removed from the canonical project. Use S1 or S2 instead.
...
4. (removed) `pipeline_hybrid_s3_h264analyze.cpp` — S3 was deprecated and removed from canonical pipelines
...
-- S1 and S2 working correctly with exact NALU counts (S3 removed)
