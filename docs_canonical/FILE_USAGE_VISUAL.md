...
<!-- S3 metadata removed from canonical file usage visual -->
║                                                                   ║
...
```
┌─────────────────────────────────────────────────────────────────┐
│ STRATEGY 1: `pipeline_hybrid_s1_h264analyze` (canonical)         │
├─────────────────────────────────────────────────────────────────┤
...
┌─────────────────────────────────────────────────────────────────┐
│ STRATEGY 2: `pipeline_hybrid_s2_h264analyze` (canonical)         │
├─────────────────────────────────────────────────────────────────┤
...
┌─────────────────────────────────────────────────────────────────┐
<!-- STRATEGY 3 removed from canonical visualization. Historic S3 artifacts live in `s3_artifacts/` if needed. -->
```
...
```
┌──────────────────────────────────────────────────────────────────┐
│ DECRYPTION LOGIC (Match encryption strategy - S1/S2)             │
├──────────────────────────────────────────────────────────────────┤
│                                                                   │
...
START
  ...
  │     │
     │     ├─ Which strategy?
     │     │  │
     │     │  ├─ Maximum security (all frames) → `pipeline_hybrid_s1_h264analyze`
     │     │  ├─ Fast streaming (I-frames only) → `pipeline_hybrid_s2_h264analyze`
     │     │  └─ Selective (S3) → `pipeline_hybrid_s3_h264analyze` (or archived S3 variants in `s3_artifacts/`)
  │     │
  ...
  │     │  │
     │     │  ├─ S1 → pipeline_hybrid_decrypt_s1_h264analyze
     │     │  ├─ S2 → pipeline_hybrid_decrypt_s2_h264analyze
     │     │  └─ S3 → pipeline_hybrid_decrypt_s3_h264analyze
  │     │
...
KEY POINTS:
• Recommended: use `*_h264analyze` pipelines (they read `nalu_info.bin` from `extract_nalu_from_h264analyze`).
• Legacy `*_fixed` binaries exist for historical/reference purposes and some S3 artifacts live in `s3_artifacts/` — they are archived and not recommended for new runs.
• NEVER use old files from `tmp_old_files/` or `old/` unless for forensic/debugging.
• ALWAYS keep the `.meta` file with the encrypted video (required for decryption).
• `.meta` file must be in the same directory as encrypted video
```
