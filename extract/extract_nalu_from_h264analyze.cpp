...
int main(int argc, char* argv[]) {
    ...
    // Write NALU info to nalu_cache/<video_basename>.nalu_info.bin
    string video_basename = input_file.substr(input_file.find_last_of("/\\") + 1);
    std::filesystem::create_directories("nalu_cache");
    string nalu_file_path = "nalu_cache/" + video_basename + ".nalu_info.bin";

    ofstream nalu_file(nalu_file_path, ios::binary);
