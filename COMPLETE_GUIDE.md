# 🎯 Hướng Dẫn Chạy Mã Hóa & Giải Mã S1, S2, S3 - Chi Tiết Đầy Đủ
...
| **S3** | ~1.2s | ~1.2s | ~18,350 (Tất cả I+P/B) | ⭐⭐⭐⭐⭐ | Nhanh |
...
**Kết quả**: 6 executables
- `pipeline_hybrid_s1_fixed` (S1 encrypt)
- `pipeline_hybrid_decrypt_s1_fixed` (S1 decrypt)
- `pipeline_hybrid_s2_fixed` (S2 encrypt)
- `pipeline_hybrid_decrypt_s2_fixed` (S2 decrypt)
- `pipeline_hybrid_s3_fixed` (S3 encrypt)
- `pipeline_hybrid_decrypt_s3_fixed` (S3 decrypt)
...
Frame [1]: Type=1 (P-frame)
  → Encrypting DC coefficients
  → NALU Type: 1
Frame [2]: Type=1 (P-frame)
  → Encrypting DC coefficients
  → NALU Type: 1
Frame [3]: Type=5 (I-frame)
  → Encrypting DC coefficients
  → NALU Type: 5
...
Frames encrypted: 18,350 (ALL I-frames + ALL P/B-frames)
Frames passed through: 1,147 (SPS, PPS, SEI only)
Written to: output.h264.s3_hybrid_fixed
Metadata written to: output.h264.s3_hybrid_fixed.meta
```

**Files Generated**:
- `output.h264.s3_hybrid_fixed` (219 MB - encrypted video)
- `output.h264.s3_hybrid_fixed.meta` (metadata)

**Kích thước**: 219M
**Thời gian**: ~1.2 giây
**Bảo mật**: Rất cao (mã hóa 18,350 NALUs - tương tự S1)
...
```bash
./pipeline_hybrid_decrypt_s1_fixed output.h264.s1_hybrid_fixed testkey
```
...
```bash
./pipeline_hybrid_decrypt_s2_fixed output.h264.s2_hybrid_fixed testkey
```
...
**Output**:
```
Reading encrypted file: output.h264.s3_hybrid_fixed
Reading metadata from: output.h264.s3_hybrid_fixed.meta
  Metadata entries: 18,350
Processing NALUs...
NALU[1]: Type=1 (encrypted)
  → Decrypting DC coefficients
  → NALU Type: 1 (P-frame)
NALU[2]: Type=1 (encrypted)
  → Decrypting DC coefficients
  → NALU Type: 1 (P-frame)
NALU[3]: Type=5 (encrypted)
  → Decrypting DC coefficients
  → NALU Type: 5 (I-frame)
...
NALUs decrypted: 18,350 (ALL I-frames + ALL P/B-frames)
NALUs passed through: 1,147 (SPS, PPS, SEI)
Written to: output.h264.s3_hybrid_fixed.decrypted
```

**Files Generated**:
- `output.h264.s3_hybrid_fixed.decrypted` (219 MB)
...
for STRATEGY in s1 s2 s3; do
    ...
    g++ -std=c++11 -O2 -o pipeline_hybrid_${STRATEGY}_fixed \
        ...
        hybrid_encryption.cpp \
        dc_metadata.cpp
    g++ -std=c++11 -O2 -o pipeline_hybrid_decrypt_${STRATEGY}_fixed \
        pipeline_hybrid_decrypt_${STRATEGY}_fixed.cpp \
        hybrid_encryption.cpp \
        dc_metadata.cpp
    ...
    time ./pipeline_hybrid_decrypt_${STRATEGY}_fixed "${VIDEO}.${STRATEGY}_hybrid_fixed" "$PASSWORD"
...
g++ -std=c++11 -O2 -o pipeline_hybrid_decrypt_s1_fixed \
    ...
    hybrid_encryption.cpp \
    dc_metadata.cpp
echo "✅ Compiled: pipeline_hybrid_decrypt_s1_fixed"
...
| **Decrypt S1** | `./pipeline_hybrid_decrypt_s1_fixed input.h264.s1_hybrid_fixed pwd` | `.decrypted` |
